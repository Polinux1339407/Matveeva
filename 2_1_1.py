...
area_salary_dic = {'Москва': 57354,
                   'Санкт-Петербург': 46291,
                   'Новосибирск': 41580,
                   'Екатеринбург': 41091,
                   'Казань': 37587,
                   'Самара': 34091,
                   'Нижний Новгород': 33637,
                   'Ярославль': 32744,
                   'Краснодар': 32542,
                   }
area_count_dic = {'Москва': 0.4581,
                  'Санкт-Петербург': 0.1415,
                  'Нижний Новгород': 0.0269,
                  'Казань': 0.0266,
                  'Ростов-на-Дону': 0.0234,
                  'Новосибирск': 0.0202,
                  'Екатеринбург': 0.0143,
                  'Воронеж': 0.014,
                  'Самара': 0.0133,
...
def as_text(value_xlsx):
    if value_xlsx is None:
        return ""
    return str(value_xlsx)


print("Динамика уровня зарплат по годам: {0}".format(str(years_salary_dic)))
print("Динамика количества вакансий по годам: {0}".format(str(years_count_dic)))
print("Динамика уровня зарплат по годам для выбранной профессии: {0}".format(str(years_salary_vac_dic)))
print("Динамика количества вакансий по годам для выбранной профессии: {0}".format(str(years_count_vac_dic)))
print("Уровень зарплат по городам (в порядке убывания): {0}".format(str(area_salary_dic)))
print("Доля вакансий по городам (в порядке убывания): {0}".format(str(area_count_dic)))
wor_bk = Workbook()
del wor_bk['Sheet']
sheet = wor_bk.create_sheet('Статистика по годам')
...
sheet = wor_bk.create_sheet('Статистика по городам')
sheet["A1"] = "Город"
sheet["A1"].border = Border(top=thin, left=thin, right=thin, bottom=thin)
sheet["A1"].font = Font(bold=True)
